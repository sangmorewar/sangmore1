...
async def bang(ctx):
    embed = discord.Embed(title="📊 BẢNG ĐIỆN TỬ TRỰC TUYẾN", color=discord.Color.blue())
    
    # Sắp xếp theo giá tăng dần
    sorted_stocks = sorted(market_prices.items(), key=lambda x: x[1])
    
    # Chia danh sách thành 2 nửa (mỗi bên 15 mã)
    mid_point = len(sorted_stocks) // 2 + (len(sorted_stocks) % 2 > 0)
    col1_stocks = sorted_stocks[:mid_point]
    col2_stocks = sorted_stocks[mid_point:]
    
    # Hàm tạo nội dung cho từng cột
    def make_column(stock_list):
        text = ""
        for symbol, price in stock_list:
            info = STOCKS[symbol]
            base = info["base"]
            percent = ((price - base) / base) * 100
            
            # Icon mũi tên
            if percent > 0: icon_trend = "📈" # Tăng
            elif percent < 0: icon_trend = "📉" # Giảm
            else: icon_trend = "​⚖️" # Đứng giá
            
            # Format: Icon Tên: Giá (Trend %)
            text += f"{info['icon']} **{symbol}**: `{price:,}` {icon_trend} {percent:.1f}%\n"
        return text

    # Tạo nội dung cho 2 cột
    desc_col1 = make_column(col1_stocks)
    desc_col2 = make_column(col2_stocks)
    
    # Thêm 2 cột vào Embed (inline=True để nằm ngang nhau)
    embed.add_field(name="🏛️ DANH MỤC A-M", value=desc_col1, inline=True)
    embed.add_field(name="🏛️ DANH MỤC N-Z", value=desc_col2, inline=True)
    
    embed.set_footer(text="Giá cập nhật Live mỗi 30s | Code by SangMore")
    await ctx.send(embed=embed)
